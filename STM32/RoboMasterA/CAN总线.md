...
- **数据同步**分为**硬件同步**和**再同步**：
  - **硬件同步**：
    ...
    - 如果检测到不同步，即**帧起始信号**不在**SS段**内，此时**CAN节点**会使用**硬件同步方式**进行调整，把**CAN节点**自身的**SS段**平移到**帧起始信号的跳变沿**处，此时即将**节点内部时序**和**总线时序**同步。
    - ![image-20231031170805546](https://nickaljy-pictures.oss-cn-hangzhou.aliyuncs.com/image-20231031170805546.png)
  - **再同步**：
    - **再同步**指通过检测**普通数据位**是否在**CAN节点**中位数据的**SS段**内，进而判断**节点内部时序**和**总线**是否同步。

### 2.CAN控制器介绍：

#### 2.1 CAN控制器简介：

- **STM32**的**CAN控制器**以集成的方式存在于**STM32芯片内部**，**STM32F1/F4/F7**的**CAN控制器**是**bxCAN**，**STM32H7**的**CAN控制器**是**FDCAN**，其中**bxCAN**支持**CAN 2.0A**和**CAN2.0 B Active**版本协议。
  - **CAN 2.0 A版本协议**：
    - 可以处理**标准数据帧**，不能处理**扩展数据帧**(扩展数据帧会识别错误)。
  - **CAN 2.0 B Active**：
    - 可以处理**标准数据帧**和**扩展数据帧**。
- **bxCAN**的主要特点：
  - 波特率最高可达1Mbps
  - 支持时间触发通信
    - **CAN控制器**的内部定时器可以在**CAN_TX/CAN_RX**的**帧起始位**(1bit)的**采样点**生成**时间戳**。
  - 具有三级发送邮箱
    - 用来缓存待发送的报文，最多可以缓存3个报文。
  - 具有2个三级深度的FIFO
    - **FIFO**表示**队列**，这里表示**接收队列**，每个**FIFO**最多可以存储**3条**有效报文。
  - 可变过滤器组
    - **F1系列**有**14**个**可变过滤器组**，**F4/F7系列**有**28**个**可变过滤器组**。
    - 用来对接收到的报文进行筛选，筛选出**有效报文**。

#### 2.2 CAN控制器工作模式：

- **睡眠模式**：
  - **CAN控制器**复位后，默认进入**睡眠模式**，此时**CAN总线**处于**空闲状态**，可以有效降低功耗。
- **初始化模式**：
  - **CAN控制器**进入**初始化模式**，此时可以对**CAN控制器**的相关**寄存器**进行配置。
- **正常模式**：
  - 当**CAN总线**和**CAN控制器**时序同步后，可以**接收**和**发送**数据。

#### 2.3 CAN控制器的测试模式：

- **静默模式**：
  - 当前节点只从总线上接收数据，接收完数据之后，不向总线产生应答信号。
  - 静默模式可以**统计总线的流量**。

- **环回模式**：
  - 当前节点向**总线**和**节点本身的CAN_RX**发送数据，不能从总线**接收**数据。
  - 环回模式可以用来进行**自检**。
- **环回静默模式**：
  - 当前节点只能向**节点本身的CAN_RX**发送数据，不能向总线**收发**数据。
  - 环回静默模式可以在**不影响总线**的前提下进行**自检**。

#### 2.4 CAN控制器发送处理：

- 第一步：选择**空闲状态**的邮箱，配置相关寄存器，将待发送的**数据帧**放入邮箱中，此时邮箱变为**挂号状态**。
- 第二步：邮箱等待成为**优先级最高**的邮箱，优先级变为最高后，变为**预定发送状态**。
- 第三步：当**总线**进入**空闲状态**的时候，邮箱变为**发送状态**，此时将**数据帧**发送到总线上(先发送给**CAN收发器**)，邮箱再次变为**空闲状态**。
